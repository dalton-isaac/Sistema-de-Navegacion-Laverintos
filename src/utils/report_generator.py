"""
===============================================================================
MÓDULO: GENERADOR AUTOMATIZADO DE INFORME TÉCNICO ACADÉMICO (APA 7)
...
def construir_informe(output_path=None):
    """Compila y genera el informe técnico formal en formato Word (.docx)."""
    # Intentar resolver ruta de entrada
    candidatos_input = [
        os.path.join(DOCS_DIR, 'Proyecto Final Fundamentos de Inteligencia Artificial.docx'),
        os.path.join(BASE_DIR, 'Proyecto Final Fundamentos de Inteligencia Artificial.docx'),
        'Proyecto Final Fundamentos de Inteligencia Artificial.docx'
    ]
    
    input_doc_path = None
    for cand in candidatos_input:
    ...
    COLOR_TITULO = RGBColor(26, 54, 93)     # Azul oscuro (#1A365D)
    COLOR_SUBTITULO = RGBColor(43, 108, 176) # Azul secundario (#2B6CB0)
    COLOR_TEXTO = RGBColor(26, 32, 44)       # Gris oscuro (#1A202C)
    ...
    def add_image_figure(image_name, title_text, caption_text, width_inches=6.2):
        candidatos = [
    ...
    add_body("El sistema se desarrolló íntegramente en lenguaje Python 3 bajo un diseño arquitectónico modular en paquetes (src/core, src/gui, src/utils), garantizando alta cohesión, desacoplamiento y reproducibilidad:")
    add_bullet("Módulo del Núcleo Algorítmico (src/core/): Contiene las funciones puras de búsqueda (bfs, dfs, ucs, greedy, a_star), heurísticas y catálogo de laberintos.", bold_prefix="1. ")
    add_bullet("Módulo de Interfaz Gráfica (src/gui/): Desarrollado en Tkinter/ttk, provee edición de celdas, animación paso a paso de la exploración y consulta de métricas en tiempo real.", bold_prefix="2. ")
    add_bullet("Módulo de Utilidades (src/utils/): Administra la generación de figuras comparativas de alta resolución mediante Matplotlib y la compilación automatizada del informe formal en Word.", bold_prefix="3. ")
    ...
    # --- SECCIÓN 6: COMPARACIÓN Y DISCUSIÓN ---
    ...
    add_h1("7. Estructura Técnica del Código y Despliegue")
    add_body("El proyecto se organiza bajo una arquitectura modular y limpia en paquetes Python:")
    add_bullet("src/core/: Algoritmos de búsqueda puros, heurísticas y generador de laberintos.", bold_prefix="• ")
    add_bullet("src/gui/: Interfaz gráfica interactiva en Tkinter.", bold_prefix="• ")
    add_bullet("src/utils/: Generación de gráficos Matplotlib y reporte Word APA 7.", bold_prefix="• ")
    add_bullet("main.py: Lanzador unificado de consola y entorno gráfico.", bold_prefix="• ")

    add_h2("7.1 Requisitos del Entorno de Ejecución")
    add_bullet("Lenguaje: Python 3.8 o superior.", bold_prefix="• ")
    add_bullet("Librerías estándar: tkinter, heapq, collections, time, random, copy, sys, os.", bold_prefix="• ")
    add_bullet("Librerías externas: matplotlib, numpy, python-docx (especificadas en requirements.txt).", bold_prefix="• ")

    add_h2("7.2 Modos de Ejecución")
    add_bullet("Modo Interfaz Gráfica (Por defecto):", bold_prefix="1. ")
    add_code("python main.py")
    add_bullet("Modo Consola / Benchmark Automatizado:", bold_prefix="2. ")
    add_code("python main.py --benchmark")
    add_bullet("Generación del Informe Técnico en Word:", bold_prefix="3. ")
    add_code("python main.py --report")
    ...
    if output_path is None:
        ...
        output_path = os.path.join(DOCS_DIR, 'Proyecto Final Fundamentos de Inteligencia Artificial.docx')
