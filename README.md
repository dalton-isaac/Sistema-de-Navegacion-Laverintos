...
Este repositorio contiene el desarrollo, implementación modular y benchmarking experimental de un **Sistema Inteligente de Búsqueda y Navegación en Laberintos**, diseñado para modelar y resolver el problema de planificación de trayectorias y rescate robótico autónomo (**USAR** - *Urban Search and Rescue*) en edificaciones colapsadas.
...
## 🖼️ Demostración Visual y Galería de Escenarios

| Gráfico Comparativo Oficial (4 Métricas) | Mapa Oficial (5x5) Resuelto con A* |
| :---: | :---: |
| ![Gráfico Comparativo](assets/grafico_comparativo_oficial.png) | ![Mapa Oficial](assets/mapa_visual_oficial.png) |

| Escenario Rescate Urbano (10x10) | Trampa Cóncava en U (12x12) |
| :---: | :---: |
| ![Mapa Urbano](assets/mapa_visual_urbano.png) | ![Trampa Cóncava](assets/mapa_visual_trampa.png) |

---

## 🧠 Algoritmos Implementados

| Tipo de Búsqueda | Algoritmo | Estructura de Datos | Complejidad Temporal | Complejidad Espacial | Heurística | Garantía de Optimalidad |
| :--- | :--- | :--- | :---: | :---: | :--- | :--- |
| **No Informada (Ciega)** | **BFS** (Breadth-First Search) | Cola FIFO (`collections.deque`) | $O(b^d)$ | $O(b^d)$ | Ninguna | Óptimo en número de pasos *(no en costo ponderado)* |
| **No Informada (Ciega)** | **DFS** (Depth-First Search) | Pila LIFO (`list`) | $O(b^m)$ | $O(b \cdot m)$ | Ninguna | No óptimo *(vulnerable a desvíos y ciclos)* |
| **No Informada (Ciega)** | **UCS** (Uniform Cost Search) | Min-Heap (`heapq` por $g(n)$) | $O(b^{1 + \lfloor C^*/\epsilon \rfloor})$ | $O(b^{1 + \lfloor C^*/\epsilon \rfloor})$ | Ninguna | **Óptimo en costo acumulado** |
| **Informada (Heurística)** | **Greedy** (Avara Best-First) | Min-Heap (`heapq` por $h(n)$) | $O(b^m)$ | $O(b^m)$ | Manhattan $h(n)$ | Rápido / No óptimo *(sensible a mínimos locales)* |
| **Informada (Heurística)** | **A\*** (A-Star Search) | Min-Heap (`heapq` por $f=g+h$) | $O(b^d)$ | $O(b^d)$ | Manhattan $h(n)$ | **Óptimo en costo acumulado y eficiente** |

### 📐 Función Heurística (Distancia Manhattan)
$$h(n) = |x_{\text{actual}} - x_{\text{meta}}| + |y_{\text{actual}} - y_{\text{meta}}|$$

* **Admisible:** $h(n) \le h^*(n)$ (al ser el costo unitario mínimo $c_{\min}=1$, la distancia Manhattan jamás sobrestima el costo real a la meta).
* **Consistente (Monótona):** $h(n) \le c(n, a, n') + h(n')$ (cumple la desigualdad triangular, garantizando optimalidad en la primera expansión a la meta).
...
│   ├── grafico_comparativo_oficial.png          # Figura 2x2 comparativa oficial
...
1. **Interfaz Gráfica Interactiva (Tkinter):**
   ...
   * Animación en tiempo real de la frontera de búsqueda (amarillo) y ruta solución trazada (azul).
   * Control de velocidad en milisegundos (1 ms a 150 ms) y modo de resolución instantánea.
   * Carga de presets y generador estocástico de laberintos solvables (validado con BFS).
...
* **A\* vs. UCS:** A\* explora un **25% menos de nodos** que UCS en el mapa oficial alcanzando idéntico costo óptimo (13), demostrando el beneficio de la poda dirigida por la heurística Manhattan.
* **Greedy Best-First:** Es el algoritmo más veloz en entornos abiertos sin trampas complejas, pero en presencia de obstáculos cóncavos en U puede quedar atrapado en mínimos locales y generar rutas subóptimas.
* **DFS:** Aunque requiere menor memoria en espacios profundos, en cuadrículas no dirigidas suele explorar ramas erráticas profundas, entregando la peor calidad de trayectoria (costo 15, 11 pasos).
* **BFS vs. UCS en Costos Asimétricos:** Mientras BFS trata todos los arcos con peso unitario, UCS prioriza los movimientos horizontales ($c=1$) sobre los verticales ($c=2$), asegurando siempre el menor costo energético.
